...
sapply(levels(map$pop), function(pop) {
  ...
  c(mean(temp), sd(temp))
}) %>% t %>% tbl_df -> avg.var.by.pop
colnames(avg.var.by.pop) <- c("Mean","SD")
rownames(avg.var.by.pop) <- levels(map$pop)
avg.var.by.pop["Population"] <- levels(map$pop)
avg.var.by.pop["Super"] <- super[levels(map$pop)]

barplot(unlist(avg.var.by.pop[,'Mean']), xlab = "Patient Number", ylab = "(0|1)  (1|1)  (1|0)  Sites per Patient",
        col = super.col[super[rownames(avg.var.by.pop)]], las = 2)

ggplot(avg.var.by.pop, aes(x=Population, y=Mean, color=factor(Super))) +
  geom_bar(stat = "identity") +
  geom_errorbar(ymin=avg.var.by.pop$Mean - avg.var.by.pop$SD, ymax=avg.var.by.pop$Mean + avg.var.by.pop$SD, width = 0.5) +
  ylim(0,500)
  scale_fill_manual(values = super.col[super[rownames(avg.var.by.pop)]])


ggplot(avg.var.by.pop, aes(x=dose, y=len, fill=supp)) +
